...
<body>
	<%
		...
		if (user.getUserID() == null || user.getUserPassword() == null || user.getUserPasswordcheck() == null || 
		...
		else if ( !( user.getUserPassword().equals(user.getUserPasswordcheck()) ) ) { // 비밀번호 != 비밀번호 확인
			PrintWriter script = response.getWriter();
		
			script.println("<script>");
			script.println("alert('비밀번호와 확인용 비밀번호가 다릅니다.')");
			script.println("history.back()");
			script.println("</script>");
		}
		else {
